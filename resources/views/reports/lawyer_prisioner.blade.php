...
<head>
    ...
    <title>Advogados por Interno</title>
    ...
    <table width="100%" class="table">
        <thead>
          <tr>
            ...
            <th style="text-align: left">Número OAB</th>
            <th style="text-align: left">Unidade Prisional</th>
        ...
        <tbody>
          @foreach ($data as $item)
            <tr>
                <td>{{$item->adv_name}}</td>
                <td>{{$item->oab_number}}</td>
                <td>{{$item->prisionalUnit->name}}</td>
                <td>{{$item->prisionalUnit->acronym}}</td>
            </tr>
          @endforeach
        </tbody>
      </table>
